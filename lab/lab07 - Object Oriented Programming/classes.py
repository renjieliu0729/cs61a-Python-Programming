    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        if player.hand != [] :
            for i in range(len(player.hand)):
                if player.hand[i].cardtype == 'Staff' :
                    if player.hand[i].attack > max_attack :
                        max_attack = player.hand[i].attack
                        max_i = i

            self.attack = self.attack + max_attack
            best_card = player.hand[max_i]

            player.hand.pop(max_i)
