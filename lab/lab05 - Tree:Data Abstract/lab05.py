...
def closer_city(lat, lon, city_a, city_b):
    ...
    disa = distance(city_a,city_c)
    disb = distance(city_b,city_c)
    if disb <= disa :
...
def berry_finder(t):
    ...
    True
    """
    # base case label is berry 
    # return True 
    #recursive to iterate each node
    if label(t) == 'berry':
        return True
    else:
    ...
    1
      2
        3
          ...
          2
    """
    # base case will grow if its a leaf
    if is_leaf(t):
        return tree(label(t), [tree(i) for i in leaves])
    # recursion to find leaf to sprout
    else:
