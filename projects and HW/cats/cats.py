...
def about(topic):
    ...
    def contain_any_topic(word):
        word = [split(word) for x in word]

        for i in word:
            for j in i:
                if lower(remove_punctuation(j)) in topic:
                    return True
        return False    
    return contain_any_topic
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    if len(typed_words) == 0 and len(source_words) == 0:
        ...
        return 100.0* correct / (len(typed_words))
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    least_diff = min([diff_function(typed_word, word, limit) for word in word_list])

    if typed_word in word_list:
        return typed_word
    elif least_diff > limit:
        return typed_word
    else:
        return word_list[diff_list.index(least_diff)]
    # END PROBLEM 5
...
def feline_fixes(typed, source, limit):
    ...
    if limit < 0:
        ...
        limit = limit - max(len(typed),len(source))
...
def minimum_mewtations(start, goal, limit):
    ...
    if not start and not goal:
    ...
    elif len(start) == "" or len(goal) == "":
        limit = limit - max(len(start),len(goal))
        ...
        sub = minimum_mewtations(start[1:],goal[1:],limit -1)
    return(min(add,remove,sub)) + 1
...
def report_progress(typed, prompt, user_id, upload):
    ...
    for i in range(min(len(typed),len(prompt))):
        if typed[i] == prompt[i]:
            ...
            break
    upload({'id': user_id, 'progress': correct_count/(len(prompt))})
...
def time_per_word(words, times_per_player):
    ...
    for i in range(len(times_per_player)):
        ...
        for j in range(1,len(times_per_player[i])) :
            sub_list.append(times_per_player[i][j] - times_per_player[i][j-1])
        result_list.append(sub_list)
    
    match_dict = match(words,result_list)
    # END PROBLEM 9
    return match_dict
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            if time(match,j,i) < min_time:
                ...
                min_idx = j
        res[min_idx].append(get_word(match,i))
    return res
