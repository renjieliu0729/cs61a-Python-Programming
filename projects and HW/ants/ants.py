...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if place.ant.name != 'Container' and self.can_contain(place.ant):
                self.store_ant(place.ant)
                place.ant = self
            elif place.ant.name == 'Container' and place.ant.can_contain(self):
                place.ant.store_ant(self)
            elif self.can_contain(place.ant) == False and place.ant.can_contain(self) == False:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        if self.place.bees != [] and self.place.is_hive == False:
            bee_list = self.place.bees
        else:
            while bee_list == [] and next_place.entrance != None:
                next_place = next_place.entrance
                if next_place.is_hive == False:
                    count_place = count_place + 1
                    if self.lower_bound <= count_place <= self.upper_bound :
                        bee_list = next_place.bees

        return random_bee(bee_list)  # REPLACE THIS LINE
        # END Problem 3 and 4
...
class HungryAnt(Ant):
    ...
    def action(self,gamestate) :
        ...
        elif self.place.bees[:] != [] and self.turns_to_chew == 0:
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        if self.ant_contained is None and other.name != 'Container':
            return True
        else:
            return False
