...
def totals_tree(m):
    ...
    if is_planet(m):
        ...
        left_end, right_end = end(left(m)), end(right(m))
        return tree(total_weight(left_end)+total_weight(right_end),[totals_tree(left_end),
                                                                    totals_tree(right_end)])
...
def replace_loki_at_leaf(t, lokis_replacement):
    ...
    True
    """
    # Base case if its leaf return a tree with label loki
    if is_leaf(t):
        if label(t) == 'loki':
...
def has_path(t, word):
    ...
    # need to return True of False
    if len(word) == 1:
        return str(label(t)) == word[0]
    
    flag = False
    # if the current label is the same then advance to next characters
    if label(t) == word[0]:
        for branch in branches(t):
            if label(branch) == word[1]:
                flag = has_path(branch, word[1:])
    return flag
